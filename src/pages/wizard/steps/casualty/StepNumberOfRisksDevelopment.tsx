...
const SHEET = 'Number of Risks Development';
...
export default function StepNumberOfRisksDevelopment() {
  const { submissionId } = useParams();
  const [{ keys, columns: yearCols }] = useState(() => genYears(12));
  const [rows, setRows] = useState<Row[]>([
    { metric: 'Number of Risks', ...Object.fromEntries(keys.map((k) => [k, ''])) },
  ]);
  ...
  return (
    <div className="space-y-6">
      <div className="rounded shadow p-4 bg-white dark:bg-gray-800">
        <h3 className="font-semibold mb-3">Number of Risks Development</h3>
        <FormTable<Row>
