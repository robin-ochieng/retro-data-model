...
export default function StepLargeLossList() {
  ...
  const columns = useMemo(() => {
    ...
    // Optional splits
    const opt: any[] = [];
    opt.push({ key: 'qs_cession', label: 'QS CESSION', type: 'number', step: '0.01', min: 0 });
    opt.push({ key: 'net_of_proportional', label: 'NET OF PROPORTIONAL', type: 'number', step: '0.01', min: 0 });
    opt.push({ key: 'xol_payment', label: 'XOL PAYMENT', type: 'number', step: '0.01', min: 0 });
    return [...base, ...opt];
  }, []);
