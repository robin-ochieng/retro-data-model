...
// Data model: store entire payload in sheet_blobs with sheet_name = 'Aggregate Triangulation (Property)'.
...
export default function StepTriangulation() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data } = await supabase
        ...
        .eq('sheet_name', 'Aggregate Triangulation (Property)')
        .maybeSingle();
  ...
  useAutosave({ years, devMonths, sections }, async (val) => {
    ...
    await supabase
      .from('sheet_blobs')
      .upsert({ submission_id: submissionId, sheet_name: 'Aggregate Triangulation (Property)', data: val, updated_at: new Date().toISOString() }, { onConflict: 'submission_id,sheet_name' });
